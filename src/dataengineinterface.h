...
// VOFA+ public protocol-engine ABI. Keep this header layout compatible with
// https://github.com/je00/Vodka/tree/master/dataengines/shared.
class RawImage
{
public:
    RawImage() {}
    ...
    {
        data_.resize(len);
        std::memcpy(data_.data(), data, static_cast<size_t>(len));
        length_ = len;
        format_ = format;
        width_ = width;
        height_ = height;
